...
const DepartmentData = () => {
  ...
  const filteredDataJabatan = departmentData.filter((designation) => {
    const { desigantion_name } = designation;
    const keyword = searchKeyword.toLowerCase();
    return desigantion_name.toLowerCase().includes(keyword);
  });
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteDataJabatan(id)).then(() => {
          Swal.fire({
            title: "Success",
            text: "Job data has been successfully deleted.",
            icon: "success",
            timer: 1000,
  ...
  useEffect(() => {
    dispatch(getDataDepartment(startIndex, endIndex));
        ...
        <div className="mt-4 flex flex-col items-center justify-between md:flex-row md:justify-between">
          <div className="flex items-center space-x-2">
            <span className="text-gray-5 dark:text-gray-4 py-4 text-sm">
              ...
              {Math.min(endIndex, filteredDataJabatan.length)} dari{" "}
              {filteredDataJabatan.length} Data Jabatan
            </span>
          </div>
